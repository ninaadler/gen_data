...
geno_subset = geno_subset %>% select(-rs3813199_G, -rs11804831_T, -rs3128342_C)
...
The regression coefficient for rs1861 under the addititve model is 0.974 (p = 1.62e-82). The positive coefficient means that each additional copy of the C allele is associated with an increased chance of having the phenotype. The low p-value means that this association is statistically significant.
...
The AIC for the additive model is 11276.91 and the AIC for the recessive model is 11291.74. While these values are very similar, the AIC for the additive model is slightly lower meaning that it has a slightly better fit.
